"use client";
import { motion } from "framer-motion";
import Link from "next/link";
import { LogoVariants } from "./Logo";
import AnimatedBackground from "./AnimatedBackground";

// Componente de icono de WhatsApp reutilizable
const WhatsAppIcon = ({ className = "w-5 h-5 sm:w-4 sm:h-4" }: { className?: string }) => (
  <svg className={className} fill="currentColor" viewBox="0 0 24 24">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.787"/>
  </svg>
);

export default function Header() {
  const handleWhatsAppClick = () => {
    // Placeholder para el número de WhatsApp - reemplazar con el número real
    const phoneNumber = "5491234567890"; // Reemplazar con tu número
    const message = "¡Hola! Me interesa conocer más sobre sus servicios de desarrollo web. ¿Podrían brindarme información?";
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  };

  return (
    <header className="bg-white/90 backdrop-blur-md border-b border-blue-200 sticky top-0 z-50 relative">
      <AnimatedBackground />
      <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center relative z-10">
        <motion.div
          whileHover={{ scale: 1.02 }}
          transition={{ type: "spring", stiffness: 400, damping: 17 }}
        >
          <LogoVariants.Header />
        </motion.div>
        <nav className="hidden md:flex space-x-8 items-center">
          <motion.a 
            href="#servicios" 
            whileHover={{ scale: 1.05, color: "#1d4ed8" }}
            className="text-gray-600 hover:text-blue-700 transition-colors font-medium"
          >
            Servicios
          </motion.a>
          <motion.a 
            href="#clientes" 
            whileHover={{ scale: 1.05, color: "#1d4ed8" }}
            className="text-gray-600 hover:text-blue-700 transition-colors font-medium"
          >
            Clientes
          </motion.a>
          <motion.a 
            href="#contacto" 
            whileHover={{ scale: 1.05, color: "#1d4ed8" }}
            className="text-gray-600 hover:text-blue-700 transition-colors font-medium"
          >
            Contacto
          </motion.a>
          <Link href="/faq">
            <motion.span 
              whileHover={{ scale: 1.05, color: "#1d4ed8" }}
              className="text-gray-600 hover:text-blue-700 transition-colors font-medium cursor-pointer"
            >
              FAQ
            </motion.span>
          </Link>
          <motion.a 
            href="https://www.pmdevop.com" 
            target="_blank" 
            rel="noopener noreferrer"
            whileHover={{ 
              scale: 1.05,
              backgroundColor: "rgba(251, 146, 60, 0.1)",
              borderColor: "rgb(251, 146, 60)"
            }}
            className="bg-gray-100/60 backdrop-blur-sm border-2 border-gray-200/80 text-gray-700 hover:text-orange-600 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:bg-orange-50/80 hover:border-orange-300 hover:shadow-lg hover:shadow-orange-500/20"
          >
            ✨ pmdevop.com ↗
          </motion.a>
        </nav>
        <motion.button
          onClick={handleWhatsAppClick}
          whileHover={{ 
            scale: 1.05,
            boxShadow: "0 10px 25px rgba(34, 197, 94, 0.3)"
          }}
          whileTap={{ scale: 0.95 }}
          className="bg-gradient-to-r from-green-600 to-green-700 text-white px-3 sm:px-6 py-2 sm:py-2 rounded-full font-medium hover:from-green-700 hover:to-green-800 transition-all duration-300 shadow-lg relative z-10 cursor-pointer flex items-center space-x-1 sm:space-x-2"
        >
          <WhatsAppIcon className="w-4 h-4 sm:w-4 sm:h-4" />
          <span className="hidden sm:inline text-sm sm:text-base">¡Envíanos un Mensaje!</span>
          <span className="sm:hidden text-xs">WhatsApp</span>
        </motion.button>
      </div>
    </header>
  );
}
